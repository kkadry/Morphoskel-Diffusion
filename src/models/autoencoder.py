...
class AttentionBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.use_flash_attention:
            ...
            # backend={'enable_math': True,'enable_flash': False, 'enable_mem_efficient': True}
    ...
    def __init__(
        ...
        for i in range(len(num_channels)):
            ...
            if not is_final_block:
                blocks.append(
                    Downsample(spatial_dims=spatial_dims, in_channels=input_channel)
                )
...
class AutoencoderKL(nn.Module):
    ...
    def __init__(
        ...
        if any((out_channel % norm_num_groups) != 0 for out_channel in num_channels):
            raise ValueError(
                "AutoencoderKL expects all num_channels being multiple of norm_num_groups"
            )

        if len(num_channels) != len(attention_levels):
            raise ValueError(
                "AutoencoderKL expects num_channels being same size of attention_levels"
            )
    ...
    def get_ldm_inputs(self, x: torch.Tensor) -> torch.Tensor:
        return self.encode_stage_2_inputs(x)
...
def prepare_VAE(VAE_cfg, VAE_checkpoint):
    """
    Prepare and initialize a Variational Autoencoder (VAE) model.

    This function creates a VAE instance based on the provided configuration
    and loads a checkpoint if specified.

    Args:
        VAE_cfg (OmegaConf): Configuration for the VAE model.
        VAE_checkpoint (OmegaConf): Configuration for the VAE checkpoint.

    Returns:
        torch.nn.Module: Initialized and potentially loaded VAE model.

    Notes:
        - The function uses `instantiate` to create the VAE model from the configuration.
        - If a checkpoint path is provided, it loads the model weights from the checkpoint.
    """
    # Creating variational autoencoder
    VAE = instantiate(VAE_cfg.params)

    # Loading checkpoint
    if VAE_checkpoint.path is not None:
        VAE = load_model_checkpoint(VAE, VAE_checkpoint.folder, VAE_checkpoint.path)
